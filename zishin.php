 ...
 * 地震botツイート
 * @author mitsugogo
 * @version 1.3 (2014/11/02)
 * @history 
 *   1.3 (2014/11/02) im.kayac.comへの通知を追加
 *   1.2 (2014/10/26) ログ吐き出し追加
 *   1.1 (2014/10/25) Logging追加
 *   1.0 (2014/10/22) 初版リリース
 **/
...
//			$stmt->bindValue(":zishinId", $zishinId);
//			$stmt->bindValue(":tweetId", @$response['id']);
//			$stmt->bindValue(":text", $tweet);
//			$stmt->bindValue(":errorFlag", (int)(isset($response['errors'])) );
//			$stmt->bindValue(":errorCode", @$response['errors']['code']);
//			$stmt->bindValue(":errorMessage", @$response['errors']['message']);
//			$stmt->bindValue(":tweetDate", @$response['created_at']);
//
//			$stmt->execute();
		} catch (Exception $ex) {
			echo $ex->getMessage()."\n";
